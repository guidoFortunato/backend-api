...
const addMovie = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ ok: false, message: "Error al añadir película", error });
  }
};
...
const getMovies = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ ok: false, message: "Error al obtener películas", error });
  }
};
...
const updateMovie = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ ok: true, message: "Película actualizada exitosamente", movie });
  } catch (error) {
    res
      .status(500)
      .json({ ok: false, message: "Error al actualizar película", error });
  }
};
...
const deleteMovie = async (req, res) => {
  ...
  try {
    ...
    await User.updateOne({ _id: movie.user }, { $pull: { movies: movie._id } });
    ...
    res
      .status(500)
      .json({ ok: false, message: "Error al eliminar película", error });
  }
};
